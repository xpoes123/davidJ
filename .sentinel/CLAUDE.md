...
Auto-maintained by Sentinel's memory system. Last updated: 2026-04-24 11:21 UTC
...
- fetch() without .ok check silently accepts 404/500 responses (confidence: 27)
- Broken asset paths fail silently—check browser console for 404s (confidence: 26)
- innerHTML is XSS-prone; prefer textContent or createElement (confidence: 22)
- Bare catch blocks hide critical errors from debugging (confidence: 18)
...
- Copy-pasted code often contains stale references—verify each instance (confidence: 12)
...
- Test asset paths work before commit (confidence: 27)
- Security-first approach to XSS prevention (confidence: 22)
...
- Throw descriptive errors with HTTP status codes (confidence: 9)
- Automated scanning followed by manual review and PR (confidence: 7)

## Learned Patterns

- Always prepend relative paths for assets (img/, css/, js/) (confidence: 25)
...
- Scan .git/config for exposed tokens before commits (confidence: 21)
- Audit for subtle visual inconsistencies across multiple pages (confidence: 19)
...
- Use descriptive alt text matching actual content, not generic placeholders (confidence: 12)
