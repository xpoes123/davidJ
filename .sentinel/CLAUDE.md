...
Auto-maintained by Sentinel's memory system. Last updated: 2026-04-24 11:23 UTC
...
- fetch() without .ok check silently accepts 404/500 responses (confidence: 30)
- Broken asset paths fail silently—check browser console for 404s (confidence: 27)
- innerHTML is XSS-prone; prefer textContent or createElement (confidence: 23)
- Bare catch blocks hide critical errors from debugging (confidence: 20)
- Copy-pasted code often contains stale references—verify each instance (confidence: 13)
...
- Test asset paths work before commit (confidence: 28)
- Security-first approach to XSS prevention (confidence: 23)
- Explicit HTTP status validation in all fetch chains (confidence: 21)
...
- Throw descriptive errors with HTTP status codes (confidence: 11)
- Accessibility: descriptive alt text and motion preferences respected (confidence: 10)
- Automated scanning followed by manual review and PR (confidence: 8)

## Learned Patterns

- Check response.ok before parsing fetch responses (confidence: 29)
- Always prepend relative paths for assets (img/, css/, js/) (confidence: 26)
- Use textContent + createElement instead of innerHTML for dynamic content (confidence: 23)
- Scan .git/config for exposed tokens before commits (confidence: 22)
- Audit for subtle visual inconsistencies across multiple pages (confidence: 20)
...
- Use descriptive alt text matching actual content, not generic placeholders (confidence: 13)
...
- Check for existing implementations before building from scratch (confidence: 6)
